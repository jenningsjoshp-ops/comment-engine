    ...
    .upsert(
      {
        ...
        daily_goal: profile.dailyGoal || 10,
...
export async function loadUser(email) {
  ...
  return {
    ...
    lastGoalDate: data.last_goal_date || null,
  };
}
...
// Save goal progress (streak + last goal date — called programmatically, not from Settings)
export async function saveGoalProgress(userId, { streak, lastGoalDate }) {
  const { error } = await supabase
    .from('users')
    .update({ streak, last_goal_date: lastGoalDate })
    .eq('id', userId);
  if (error) console.error('Save goal progress error:', error);
}

// Log an error to Supabase for debugging
export async function logError({ screen, action, message, userId = null }) {
  try {
    ...
    // Never crash on error logging
  }
}

// Save user feedback
export async function saveFeedback({ type, message, user, email, igHandle, tier }) {
  const { error } = await supabase
    .from('feedback')
    .insert({
      user_name: user,
